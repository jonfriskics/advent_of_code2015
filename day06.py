...
for rules in input.strip().split("\n"):
  instruction_match = re.search("^(\D+)(\d+,\d+)\s\w+\s(\d+,\d+)", rules)
  
  instruction = instruction_match.groups()[0].strip()
  start_x = int(instruction_match.groups()[1].split(",")[0].strip())
  start_y = int(instruction_match.groups()[1].split(",")[1].strip())

  end_x = int(instruction_match.groups()[2].split(",")[0].strip())
  end_y = int(instruction_match.groups()[2].split(",")[1].strip())
